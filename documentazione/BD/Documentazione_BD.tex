...
\begin{lstlisting}[style=SQL, caption={\texttt{search\_notifications(SHORT\_TEXT, TEXT, INT, BOOLEAN)}}]
CREATE OR REPLACE FUNCTION search_notifications(
	nome_destinatario SHORT_TEXT,
...
\textbf{\\\\Esempio:}
\begin{lstlisting}[style=SQL]
SELECT * FROM search_notifications('giuseppe', 'Pagina', 1, FALSE);
\end{lstlisting}
Questa query restituirà tutte le notifiche dell'utente "giuseppe" relative alle pagine che contengono un titolo \textit{simile} a "Pagina", di tipo 1 \texttt{UPDATE\_ACCEPTED} e che non sono state ancora lette dal destinatario.
...
\subsubsection{Funzione \texttt{search\_notifications(SHORT\_TEXT, TEXT, INT)}}

\begin{lstlisting}[style=SQL, caption={\texttt{search\_notifications(SHORT\_TEXT, TEXT, INT)}}]
CREATE OR REPLACE FUNCTION search_notifications(
	nome_destinatario SHORT_TEXT,
	titolo_pagina_text TEXT,
	tipo_notifica INT
...
\textbf{\\\\Esempio:}
\begin{lstlisting}[style=SQL]
SELECT * FROM search_notifications('mario', 'Pagina', 2);
\end{lstlisting}
Questa query restituirà tutte le notifiche dell'utente "mario" relative alle pagine che contengono un titolo \textit{simile} a "Pagina", di tipo 2 \texttt{UPDATE\_REJECTED}. Ritorna sia le pagine lette che quelle non lette.

\newpage


\subsubsection{Funzione \texttt{search\_notifications(SHORT\_TEXT, TEXT, BOOLEAN)}}

\begin{lstlisting}[style=SQL, caption={\texttt{search\_notifications(SHORT\_TEXT, TEXT, BOOLEAN)}}]
CREATE OR REPLACE FUNCTION search_notifications(
	nome_destinatario SHORT_TEXT,
	titolo_pagina_text TEXT,
	notifica_letta BOOLEAN
...
\textbf{\\\\Esempio:}
\begin{lstlisting}[style=SQL]
SELECT * FROM search_notifications('mario', 'Pagina', FALSE);
\end{lstlisting}
Questa query restituirà tutte le notifiche dell'utente "mario" relative alle pagine che contengono un titolo \textit{simile} a "Pagina", che non sono state ancora lette.

\newpage



\subsubsection{Funzione \texttt{search\_notifications(SHORT\_TEXT, TEXT)}}

\begin{lstlisting}[style=SQL, caption={\texttt{search\_notifications(SHORT\_TEXT, TEXT)}}]
CREATE OR REPLACE FUNCTION search_notifications(
	nome_destinatario SHORT_TEXT,
	titolo_pagina_text TEXT
) RETURNS TABLE (
...
\textbf{\\\\Esempio:}
    \begin{lstlisting}[style=SQL]
SELECT * FROM search_notifications('prova', 'Pagina');
	\end{lstlisting}
Questa query restituirà tutte le notifiche dell'utente "prova" relative alle pagine che contengono un titolo \textit{simile} a "Pagina", a prescindere dal tipo della notifica e dalla visualizzazione della notifica (letta o non);
